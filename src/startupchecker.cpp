...
{
    QString message = "The application requires some components to process images with LVGL:\n\n";
    ...
    message += "This is required for image processing functionality.";
...
bool StartupChecker::downloadAndSetupComponents(const MissingComponents& missing)
{
    ...
    // Download LVGL if needed
    if (missing.needsLVGL) {
        qDebug() << "Setting up LVGL library...";
        if (!m_libraryChecker->checkAndDownloadLibraries()) {
            qDebug() << "Failed to setup LVGL library";
            allSuccess = false;
        } else {
            qDebug() << "LVGL library setup completed";
        }
    }
    ...
    if (allSuccess) {
        QMessageBox::information(m_parent, "Setup Complete", 
                               ...
                               "The application is ready for image processing.");
    } else {
        QMessageBox::warning(m_parent, "Setup Incomplete", 
                           ...
                           "Image processing functionality may not work correctly.");
    }
