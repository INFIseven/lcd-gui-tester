    ...
    , m_scriptRunner(nullptr)
{
    ...
    if (!m_startupChecker->performStartupCheck()) {
        ...
        QMessageBox::warning(this, "Setup Incomplete",
                           "Some components are missing. Image processing functionality may not work.");
    }
...
void MainWindow::addImage(const QString& imagePath)
{
    if (m_images.size() >= MAX_IMAGES) {
        QMessageBox::warning(this, "Error", QString("Maximum %1 images allowed!").arg(MAX_IMAGES));
        return;
    }
...
void MainWindow::updateUI()
{
    ...
    // Update counter
    m_counterLabel->setText(QString("Images: %1/%2").arg(m_images.size()).arg(MAX_IMAGES));
...
bool MainWindow::validateImageSize(const QString& imagePath)
{
    ...
    if (pixmap.isNull()) {
        QMessageBox::critical(this, "Error", QString("Failed to load image: %1").arg(imagePath));
        return false;
    }
